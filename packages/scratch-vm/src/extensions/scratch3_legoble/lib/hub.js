...
class Hub {
    ...
    _onConnect () {
        ...
        this._firstNotificationCallback = () => {
            this.sendMessage(MessageType.HUB_PROPERTIES,
                [HubPropertyReference.ADVERTISING_NAME, HubPropertyOperation.ENABLE_UPDATES], false);
            this.sendMessage(MessageType.HUB_PROPERTIES,
    ...
    _startPollingBatteryLevel () {
        this.sendMessage(MessageType.HUB_PROPERTIES,
            [HubPropertyReference.BATTERY_VOLTAGE, HubPropertyOperation.REQUEST_UPDATE]);
    ...
    motorRunForDegrees (portId, direction, degrees) {
        ...
        if (motor && motor.canUseSpeed) {
            const speed = motor.speed * direction;
    ...
    inputValue (portId, key) {
        const device = this._devices[portId];
        if (device && Object.prototype.hasOwnProperty.call(device.inputValues, key)) {
    ...
    internalInputValue (key) {
        for (const [portId, device] of Object.entries(this._devices)) {
            if (portId >= 0x32 && Object.prototype.hasOwnProperty.call(device.inputValues, key)) {
                return device.inputValues[key];
            }
        }
        return null;
    ...
    setVolume (volume) {
        ...
        return this.sendMessage(MessageType.HUB_PROPERTIES,
            [HubPropertyReference.SPEAKER_VOLUME, HubPropertyOperation.SET, volume]);
    }
}
