...
class Scratch3RoboHatOneGPIO {
    ...
    mpu_read (args){
        ...
        if (!connected) {
            const callbackEntry = [this.mpu_read.bind(this), args];
            wait_open.push(callbackEntry);
        } else {
            if (!mpu_initialized) {
    ...
    ina_read (args){
        ...
        if (!connected) {
            const callbackEntry = [this.ina_read.bind(this), args];
            wait_open.push(callbackEntry);
        } else {
            if (!ina_initialized) {
    ...
    connect () {
        ...
        const url = `ws://${ws_ip_address}:9005`;
        console.log(url);
